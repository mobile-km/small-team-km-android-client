...
public class ScrollLayout extends ViewGroup {
	...
	public void snapToScreen(int whichScreen) {
		...
		if (getScrollX() != (whichScreen * getWidth())) {
			...
			if (whichScreen == 0 && mCurScreen == 1) {
				mScroller.startScroll(getScrollX(), 0, delta + 40, 0,
						Math.abs(delta) * 2);
				System.out.println("else = " + getScrollX() + " : " + delta
						+ 40 + " : " + Math.abs(delta) * 2 + " : "
						+ whichScreen);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			// System.out.println("mCurScreen : deltax : countMIX = " +
			// mCurScreen + " : " + deltaX + " : " + countMIX);
		...
		case MotionEvent.ACTION_UP:
			...
			} else if (mCurScreen == 1 && (x < 60 && x > 0)
					&& (y > 0 && y < 60)) {
				snapToScreen(mCurScreen - 1);
			} else if (mCurScreen == 0
					&& (x < getChildAt(0).getMeasuredWidth() && x > getChildAt(
							0).getMeasuredWidth() - 40) && (y > 0 && y < 50)) {
				countMIX = 0;
				snapToScreen(mCurScreen + 1);
			} else {
